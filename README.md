...
var d = ForwardDiff<double>.Derivative(
    x => DualFunctions<double>.Sin(x * x) + x * x * x,
    1.5);
```
...
var grad = ForwardDiff<double>.Gradient(
    v => v[0] * v[0] * v[1] + DualFunctions<double>.Exp(v[1]),
    new[] { 1.5, 0.3 });
...
var z = x * x + ReverseFunctions<double>.Sin(x * y);
tape.Backward(z);
...
var jv = ForwardJacobian.Compute<double>(
    d => new[] { d[0] * d[1], d[0] + d[1] },
    x: new[] { 2.0, 3.0 },
    tangent: new[] { 1.0, 0.0 });

// VJP: vᵀ·J in one backward pass
var vj = ReverseJacobian.Compute<double>(
    vars => new[] { vars[0] * vars[1], vars[0] + vars[1] },
    x: new[] { 2.0, 3.0 },
    cotangent: new[] { 1.0, 1.0 });

// Full Jacobian via N forward passes
var J = ReverseJacobian.Jacobian<double>(
    d => new[] { d[0] * d[0] + d[1], DualFunctions<double>.Sin(d[0]) * d[1] },
    new[] { 1.3, 0.7 });
```
...
The symbolic `ExpressionInterpreter` is the AOT-safe expression evaluator.
